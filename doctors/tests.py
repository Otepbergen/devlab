...
class Activity3Test(StaticLiveServerTestCase):
    ...
    def test_reviews_logo_get(self):
        ...
        with open(os.path.join(current_dir, 'static', 'logo.png'), 'rb') as f:
    ...
    def test_base_html_content(self):
        ...
        """
        base_template = read_content(os.path.join(settings.BASE_DIR, 'templates', 'main.html'))
    ...
    def test_review_base_extends_main(self):
        """Test that the reviews main.html extends from the main.html"""
        reviews_base = read_content(os.path.join(settings.BASE_DIR, 'reviews', 'templates', 'reviews', 'main.html'))
        self.assertTrue(reviews_base.startswith('{% extends \'main.html\' %}'))

    # def test_reviews_templates_extend_reviews_base(self):
    #     """Test that the reviews templates extends the reviews main.html file."""
    #     book_detail_template = read_content(os.path.join(settings.BASE_DIR, 'reviews', 'templates', 'reviews',
    #                                                      'book_detail.html'))
    #     self.assertTrue(book_detail_template.startswith('{% extends \'reviews/main.html\' %}'))
    #
    #     book_list_template = read_content(os.path.join(settings.BASE_DIR, 'reviews', 'templates', 'reviews',
    #                                                    'book_list.html'))
    #     self.assertTrue(book_list_template.startswith('{% extends \'reviews/main.html\' %}'))
